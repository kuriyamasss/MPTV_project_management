...
const ProjectBoard: React.FC = () => {
  ...
  const fetchProject = async () => {
    try {
      ...
      message.error('èŽ·å–é¡¹ç›®è¯¦æƒ…å¤±è´¥');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateTask = async (values: any) => {
    ...
    try {
      ...
      message.success('ä»»åŠ¡åˆ›å»ºæˆåŠŸ');
  ...
  return (
    <Layout style={{ height: '100vh' }}>
        ...
        <div style={{ position: 'absolute', left: '50%', transform: 'translateX(-50%)' }}>
          <Segmented
            options={[
              ...
              { label: 'ç”˜ç‰¹å›¾', value: 'gantt', icon: <BarsOutlined /> },
      ...
      <Modal title="æ–°å»ºä»»åŠ¡" open={isModalOpen} onCancel={() => setIsModalOpen(false)} onOk={form.submit}>
        <Form form={form} layout="vertical" onFinish={handleCreateTask}>
          <Form.Item name="title" label="ä»»åŠ¡æ ‡é¢˜" rules={[{ required: true }]}>
            <Input />
          </Form.Item>
          <Form.Item name="description" label="æè¿°">
            <Input.TextArea />
          </Form.Item>
          ...
          <Form.Item name="start_date" label="è®¡åˆ’å¼€å§‹">
             <DatePicker showTime style={{width: '100%'}} />
          </Form.Item>
          <Form.Item name="end_date" label="è®¡åˆ’æˆªæ­¢">
             <DatePicker showTime style={{width: '100%'}} />
          </Form.Item>
        </Form>
      </Modal>
